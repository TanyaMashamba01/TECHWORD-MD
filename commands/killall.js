...
async function killAllCommand(sock, chatId, message, senderId) {
    try {
        ...
        if (nonAdmins.length === 0) {
            await sock.sendMessage(chatId, { text: '⚠️ No non-admin members to remove.' }, { quoted: message });
            return;
        }

        if (subCommand === 'confirm') {
            await executeKillAll(sock, chatId, message, nonAdmins);
            return;
        }

        await sock.sendMessage(chatId, {
            text: `⚠️ *KILL ALL WARNING*\n\nThis will remove *${nonAdmins.length}* non-admin member(s) from this group.\n\n⏳ Auto-executing in *15 seconds*...\n\n✅ Type \`.killall confirm\` to start immediately\n❌ Type \`.killall cancel\` to stop`
        }, { quoted: message });
...
async function executeKillAll(sock, chatId, message, nonAdmins) {
    try {
        pendingKillAll.delete(chatId);

        await sock.sendMessage(chatId, {
            text: `⏳ Removing ${nonAdmins.length} member(s)... This may take a moment.`
        });

        const batchSize = 5;
        let removed = 0;
        for (let i = 0; i < nonAdmins.length; i += batchSize) {
            const batch = nonAdmins.slice(i, i + batchSize).map(p => p.id);
            try {
            ...
            if (i + batchSize < nonAdmins.length) {
                await new Promise(r => setTimeout(r, 1000));
            }
        }

        await sock.sendMessage(chatId, {
            text: `✅ *Kill All Complete*\n\nRemoved ${removed}/${nonAdmins.length} non-admin member(s).`
        });
